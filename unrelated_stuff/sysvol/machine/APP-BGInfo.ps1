...
#
# Consts.
$binInstaller = Resolve-Path -ErrorAction Stop -Path ($PSScriptRoot + "\..\install\BGInfo\BGInfo_v4.32.1.msi")
$binInstallerArg = "/i `"$binInstaller`" /qb /norestart REBOOT=ReallySuppress"
$binInstalledVersionFile = $ENV:ProgramFiles + "\BGInfo\Bginfo64.packageversion"
#
# Runtime Variables.
