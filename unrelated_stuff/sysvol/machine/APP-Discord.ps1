...
#
# Consts.
$binInstaller = Resolve-Path -ErrorAction Stop -Path ($PSScriptRoot + "\..\install\Discord\Discord_v1.0.9005.0.msi")
$binInstallerArg = "/i `"$binInstaller`" /qb /norestart REBOOT=ReallySuppress"
$binInstalledVersionFile = ${ENV:ProgramFiles(x86)} + "\Discord\Discord.packageversion"
#
# Runtime Variables.
