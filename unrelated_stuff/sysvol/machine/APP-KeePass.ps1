...
#
# Consts.
$binInstaller = Resolve-Path -ErrorAction Stop -Path ($PSScriptRoot + "\..\install\KeePass\KeePass_v2.53.1.0.msi")
$binInstallerArg = ('/i "' + $binInstaller + '" /qb /norestart REBOOT=ReallySuppress')
$binInstalledExecutable = ${ENV:ProgramFiles(x86)} + "\KeePass2x\KeePass.exe"
#
# Runtime Variables.
