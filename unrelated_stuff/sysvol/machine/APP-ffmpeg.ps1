...
#
# Consts.
$binInstaller = Resolve-Path -ErrorAction Stop -Path ($PSScriptRoot + "\..\install\ffmpeg\ffmpeg_v23.08.23.msi")
$binInstallerArg = "/i `"$binInstaller`" /qb /norestart REBOOT=ReallySuppress"
$binInstalledVersionFile = $ENV:ProgramFiles + "\ffmpeg\ffmpeg.packageversion"
#
# Runtime Variables.
