...
REM Consts.
SET DO_CLEANUP=1
SET DO_ONLINE_BUILD=1
SET PRODUCT_NAME=Syncthing
REM
REM Offline build - avoids downloading anything from the internet but you need to provide the third-party files locally yourself.
IF "%DO_ONLINE_BUILD%" == "0" call :setupLocalBuildEnv
IF "%DO_ONLINE_BUILD%" == "1" call :downloadPrerequisites
REM 
REM Runtime Vars.
REM 	Input files 
...
for /f "tokens=*" %%A in ('powershell -ExecutionPolicy "ByPass" "(Get-Item -path \"%SYNCTHING_EXE%\").VersionInfo.ProductVersion" ^| SourceDir\psreplace "-rc" ""') do SET SYNCTHING_EXE_PRODUCTVERSION=%%A
...
SET "PATH=%PATH%;%SCRIPT_PATH%\SourceDir"
REM
goto :eof
